...
typedef struct dcg_node_ctx_ops {
 ...
 *
 * One rule is installed either way: a read a STORE makes carries the store's own
 * rule from birth, because that rule is the read's (see c_collections.h) and the
 * dispatch is not asked to find it.
 *
...
typedef struct dcg_node {
    ...
    // === Context Callbacks ===
    dcg_node_ctx_ops               ctx_ops;              // What entering and leaving this type means.
    // === Out Buffer ===
...
static inline dcg_node_ctx_ops               c_dcg_node_ctx_ops_for(dcg_node_type ntype);
...
static inline int c_dcg_node_init(dcg_node* node, dcg_node_type ntype, const char* repr) {
    ...
    node->ctx_ops = c_dcg_node_ctx_ops_for(ntype);
...
/**
 * @brief Enter a BINARY branching node: reserve one arm for each of its two branches.
 *
 * The two-way case, and the enter every type gets unless its own arity says
 * otherwise: a branching constant or expression, and a breakpoint, all reserve
 ...
 * @param mgr   Manager holding the build (unused: reserving is the node's own).
 * @return DCG_OK, or a DCG_ERR_* code.
 */
static inline int c_dcg_node_ctx_enter_binary(dcg_node* node, dcg_logic_group_manager* mgr) {
    (void) mgr;
    if (!node) return DCG_ERR_INVALID_ARG;
...
/**
 * @brief Enter the graph's entry point: reserve its single unconditioned arm.
 *
 * A root takes exactly one child and takes it unconditionally, so it reserves
 * one arm rather than the binary pair every other branching node reserves.
 *
 * @param node  The root being entered.
 * @param mgr   Manager holding the build (unused: reserving is the node's own).
 * @return DCG_OK, or a DCG_ERR_* code.
 */
static inline int c_dcg_node_ctx_enter_root(dcg_node* node, dcg_logic_group_manager* mgr) {
    (void) mgr;
    if (!node) return DCG_ERR_INVALID_ARG;
    ...
    int ret = c_dcg_node_append(node, arm, DCG_NO_CONDITION);
    if (ret != DCG_OK) {
        c_dcg_node_free(arm);
        return ret;
    }
...
/**
 * @brief Refuse to enter: a leaf and a stand-in hold nothing a build continues in.
 *
 * @param node  Node being entered (unused).
 * @param mgr   Manager holding the build (unused).
 * @return DCG_ERR_TYPE, always.
 */
static inline int c_dcg_node_ctx_enter_refused(dcg_node* node, dcg_logic_group_manager* mgr) {
    (void) node;
    (void) mgr;
    return DCG_ERR_TYPE;
}
...
/**
 * @brief The enter half of a type's context ops.
 *
 * The type is what decides, so this is the single place the mapping lives. The
 * special family is not uniform: a root reserves one arm instead of the binary
 * pair, a breakpoint reserves the pair like any other branching node. The exit
 * half is left NULL here and installed by the manager - see c_dcg_node_ctx_ops.
 *
 * @param ntype  Node type.
 * @return The pair, with `exit_fn` NULL.
 */
static inline dcg_node_ctx_ops c_dcg_node_ctx_ops_for(dcg_node_type ntype) {
    dcg_node_ctx_ops ops;
    ops.enter_fn = NULL;
    ops.exit_fn  = NULL;

    switch (ntype) {
        case DCG_NODE_ROOT:
            ops.enter_fn = c_dcg_node_ctx_enter_root;
            break;
        case DCG_NODE_PLACEHOLDER:
            ops.enter_fn = c_dcg_node_ctx_enter_refused;
            break;
        default:
            ops.enter_fn = c_dcg_node_type_is_action(ntype) ? c_dcg_node_ctx_enter_refused : c_dcg_node_ctx_enter_binary;
            break;
    }
    return ops;
}
...
static inline int c_dcg_node_replace_shared(dcg_node* old_node, dcg_node* new_node) {
     ...
     *
     * The join lands at the END of this parent's list rather than in the slot
     * the displaced node held. A node carries one sibling chain, and its former
     * parent walks that chain to reach it - landing it mid-list would hand that
     * parent the rest of this list to walk into, and from there back round to
     * itself. At the end, the chain it walks stops where it should.
     */
    ...
    return c_dcg_node_append(parent, new_node, condition);
}
