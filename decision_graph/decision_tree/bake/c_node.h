...
/**
 * @brief Bit masks over a node kind: family nibble and variant number.
 */
...
typedef enum dcg_node_hook_type {
    ...
    DCG_HOOK_COUNT     = 3   // Number of hook kinds (array bound).
} dcg_node_hook_type;
...
typedef enum dcg_node_event {
    DCG_NODE_EVENT_CHILD_ADDED   = 0,  // A child was linked under this node.
    DCG_NODE_EVENT_CHILD_REMOVED = 1,  // A child was unlinked (detach / replace / free).
    DCG_NODE_EVENT_MODIFIED      = 2,  // Metadata changed (repr, labels, hooks).
    DCG_NODE_EVENT_CLEARED       = 3,  // All children were dropped, the node survives.
    DCG_NODE_EVENT_FREED         = 4   // The node is being torn down.
} dcg_node_event;
...
typedef struct dcg_node {
    ...
    // === Meta Data ===
    dcg_node_type                  ntype;               // Node kind.
    const char*                    repr;                // Display text. // OWNED - a nested copy.
    uint128_t                      uid;                 // Stable identity, mirrored by the Python layer.
...
static inline dcg_node*                      c_dcg_node_find_by_uid(const dcg_node* root, uint128_t uid);
...
static inline int                            c_dcg_node_format_uid(uint128_t uid, char* out, size_t cap);
 ...
 * @param variant  Variant number (the low byte of the kind).
...
/**
 * @brief Stable display name of a node kind (the enum name without its prefix).
 *
...
/**
 * @brief Predicate: is the kind in the operator family?
 *
 * @param ntype  Node kind.
 * @return true for operator kinds.
 */
static inline bool c_dcg_node_type_is_op(dcg_node_type ntype) {
...
/**
 * @brief Predicate: is the kind in the action family?
 *
 * Action nodes are leaves: the graph terminates (or hands over control) at them.
 *
 * @param ntype  Node kind.
 * @return true for action kinds.
 */
static inline bool c_dcg_node_type_is_action(dcg_node_type ntype) {
...
/**
 * @brief Predicate: is the kind in the special family (root / breakpoint)?
 *
 * @param ntype  Node kind.
 * @return true for special kinds.
 */
static inline bool c_dcg_node_type_is_special(dcg_node_type ntype) {
...
/**
 * @brief Fixed operand count of a node kind.
 *
 * @param ntype  Node kind.
...
static inline int c_dcg_node_init(dcg_node* node, dcg_node_type ntype, const char* repr) {
    ...
    return c_dcg_node_set_repr(node, repr);
}
 ...
 * @param uid   Identity to look for.
 * @return The node, or NULL when there is none.
 */
static inline dcg_node* c_dcg_node_find_by_uid(const dcg_node* root, uint128_t uid) {
    if (!root) return NULL;
    if (c_u128_cmp(root->uid, uid) == 0) return (dcg_node*) root;
...
/**
 * @brief Render a node's uid as a 32-character hex string.
 *
 * @param uid  Identity to render.
...
static inline int c_dcg_node_format_uid(uint128_t uid, char* out, size_t cap) {
    ...
    n = snprintf(out, cap, "%016llx%016llx", (unsigned long long) (uid >> 64), (unsigned long long) uid);
#else
    n = snprintf(out, cap, "%016llx%016llx", (unsigned long long) uid.hi, (unsigned long long) uid.lo);
#endif
